...
class MonteCarloResult:
    ...
    def maxdd(self) -> Dict[str, float]:
        ...
        if self._maxdd_cache is None:
            # Per-column max drawdown via a vectorized kernel (was a Python loop
            # over columns calling pandas .cummax()). Bit-identical: growth =
            # col + 1; running_max = cummax; ((growth-rmax)/rmax).min().
            from ._kernels import max_drawdown_columns

            maxdd_values = max_drawdown_columns(self.data.to_numpy())
...
def run_montecarlo(
    ...
    """
    from ._kernels import mc_cumulative
    ...
    # Pre-generate a seeded permutation-index matrix (n_periods x sims). Column 0
    # is the identity (original, unshuffled) path; columns 1.. are independent
    # shuffles. ``rng.permutation(n)`` consumes the RNG stream identically to the
    # original ``rng.permutation(returns_array)`` and ``arr[perm] == permutation(arr)``,
    # so results are bit-identical for any given seed — while the columns are now
    # independent, enabling a parallel cumulative-return kernel.
    idx_matrix = np.empty((n_periods, sims), dtype=np.intp)
    idx_matrix[:, 0] = np.arange(n_periods)
    for i in range(1, sims):
        idx_matrix[:, i] = rng.permutation(n_periods)

    # Calculate cumulative returns for all paths: (1 + r).cumprod() - 1
    cumulative = mc_cumulative(returns_array, idx_matrix)
